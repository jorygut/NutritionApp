...
const NutritionTrends: React.FC = () => {
    ...
    const [dayLogs, setDayLogs] = useState(7); // Default to 7 days
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);
    const [selectedMacro, setSelectedMacro] = useState('calories');
    const [selectedDayRange, setSelectedDayRange] = useState(7); // Track selected day range
    ...
    const FetchLogs = async (days) => {
        ...
        try {
            const response = await axios.get('http://127.0.0.1:5000/api/updateHistory', {
                headers: {
    ...
    const GetCoach = async () =>{
        try{
            ...
            const response = await axios.get('http://127.0.0.1:5000/api/nutritionTrendsCoach', config)
