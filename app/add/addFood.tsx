...
const AddFood: React.FC = () => {
  ...
  const fetchFoodData = async () => {
    ...
    fetch(`http://127.0.0.1:5000/api/food?query=${encodeURIComponent(query)}&searchLength=${searchLength}&token=${(token)}`)
  ...
  const fetchFoodDataHistory = async () => {
    const token = await SecureStore.getItemAsync('userToken')
    fetch(`http://127.0.0.1:5000/api/foodHistory?query=${encodeURIComponent(query)}&token=${encodeURIComponent(token)}&searchLength=${searchLength}`)
      .then((response) => response.json())
      .then((data) => {
        const formattedData = data.map(item => ({
  ...
  const RetreiveRecentLogs = async () => {
    try {
      ...
      const response = await axios.get('http://127.0.0.1:5000/api/retreiveRecentHistory', config);
  ...
  const RetrieveMyFood = async () => {
    ...
    try {
      ...
      const response = await axios.get('http://127.0.0.1:5000/api/retrieveCustom', config)
      if (response.status === 200){
  ...
  const fetchFoodDataCustom = async () => {
    try {
        const token = await SecureStore.getItemAsync('userToken');
        const response = await fetch(`http://127.0.0.1:5000/api/foodCustom?query=${encodeURIComponent(query)}&token=${encodeURIComponent(token)}&searchLength=${searchLength}`);
