...
const HomeScreen: React.FC = () => {
  ...
  const retrieveMeals = async (date: string) => {
    try {
      ...
      const url = `http://127.0.0.1:5000/api/meals?activateDate=${encodeURIComponent(dateView)}&token=${token}`;
  ...
  const fetchNutrientGoals = async () => {
    ...
    try {
      const response = await axios.get('http://127.0.0.1:5000/api/fetchNutrientGoals', {
        headers: {
  ...
  const removeFood = async (mealIndex: number, foodIndex: number) => {
    ...
    Animated.timing(food.animatedValue!, {
      ...
      try {
        const response = await axios.post('http://127.0.0.1:5000/api/remove', {
          food: food
        });
  ...
  const updateMealDates = async (date) => {
    const token = await SecureStore.getItemAsync('userToken');
    try {
        const response = await axios.post('http://127.0.0.1:5000/api/update_meals', {
            date: date,
            token: token
        });
  ...
  const removeMeal = async (index: number) => {
    ...
    try{
      const token = await SecureStore.getItemAsync('userToken');
      const response = await axios.post('http://127.0.0.1:5000/api/removemeal', {
        meal: `Meal ${index + 1}`,
        date: currentDate,
        token: token,
      });
  ...
  const CoachUpdates = async () =>{
    try{
      ...
      const response = await axios.get('http://127.0.0.1:5000/api/getGoals', config)
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.calendar}>
        <Button title='Calendar' onPress={() => navigation.navigate('settings/calendar')}/>
      </View>
