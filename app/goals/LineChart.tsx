  ...
  try {
    ...
    if (response.status === 200) {
      ...
      if (Array.isArray(weights)) {
        ...
        const startDate = timeframe > 0 ? subDays(endDate, timeframe) : new Date(0); // Start from the beginning of time if timeframe is 0

        // Filter weights based on the calculated date range
        const filteredWeights = sortedWeights.filter(weight =>
          isWithinInterval(new Date(weight.date), { start: startDate, end: endDate })
        );

        // Group weights by date and calculate average
